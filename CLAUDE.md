...
# 절대 하지 말아야 할 것(주의사항)
- `application-prod.yml`에 **값을 쓰지 말 것** — 비밀번호·키 같은 실제 값은 배포 파이프라인이 K8s Secret 등으로 주입하고, 이 파일엔 `${ENV_VAR}` 참조만 둔다
  - 다만 **코드가 읽는 프로퍼티 이름이 바뀌면 이 파일도 같이 고친다**(예: `jwt.secret` → `jwt.private-key`). 안 고치면 그 서비스는 운영에서 기동조차 못 하고, 참조 이름은 값이 아니라 코드 계약의 일부다. 파이프라인 담당에게 "주입할 환경변수 이름이 바뀌었다"고 알리는 것도 잊지 말 것
- `src/main/resources/db/migration/V*.sql` 기존 파일 수정 금지, 새 버전만 추가
- main 브랜치에 강제 푸시 금지
- 다른 서비스의 DB 테이블에 직접 접근하지 말 것 (API 또는 이벤트로만 통신)
- `modules:common`에 도메인/비즈니스 로직을 추가하지 말 것. **서비스 경계를 넘는 계약 클래스만** 둔다 — 아래 세 조건을 전부 만족해야 한다:
  1. 프레임워크(Servlet/WebFlux/Spring) 타입에 의존하지 않는 순수 Java
  2. 도메인/비즈니스 로직이 없음(상수·레코드·인터페이스 수준)
  3. **서로 다른 스택의 모듈이 같은 값을 공유해야만 하는 계약** — 한쪽만 바뀌면 런타임에 조용히 깨지는 종류
  - 현재 해당하는 것: 에러·응답 계약(`ErrorCode`/`CommonErrorCode`/`BusinessException`/`DependencyFailureException`/`ErrorResponse`), 인증 헤더 계약(`AuthHeaders`)
  - `AuthHeaders`가 여기 있는 이유: 게이트웨이(WebFlux)와 서비스(Servlet)는 스택이 달라 코드를 공유할 수 없지만 헤더 이름은 반드시 일치해야 한다. 양쪽이 리터럴로 각자 들고 있다가 실제로 회원가입이 100% 실패한 전례가 있다(Phase 1, `X-Internal-Api-Key`)
  - 조건 3을 만족하지 않으면(한 서비스만 쓰면) 그 서비스에 둔다
