...
/**
 * 게이트웨이가 아직 없어 서명 검증 없이 X-Account-Id 헤더를 그대로 신뢰한다
 * (member-service/project-service CurrentMemberArgumentResolver와 동일한 임시 조치, 내부망 전제).
 * 게이트웨이가 JWT를 파싱해 이 헤더를 주입하게 되면 이 리졸버는 그대로 재사용 가능 —
 * 게이트웨이 구현 시 "헤더가 신뢰 가능한 경로로만 들어오는지" 반드시 재검증할 것.
 *
 * 헤더 값을 신뢰하는 것과 리소스 소유권을 대조하는 것은 별개다(security.md S4) —
 * 이 리졸버는 "누가 요청했는지"만 알려주고, "그 사람이 이 리소스의 주인인지"는
 * 각 애플리케이션 서비스가 매번 별도로 검증해야 한다.
 */
@Component
