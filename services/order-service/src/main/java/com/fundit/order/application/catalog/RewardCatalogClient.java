 ...
 * `GET /api/v1/projects/{projectId}/rewards`, ProjectDomainApiSpec.md #14 — projectId는
 * publicId(UUID) 기준). 재고(잔여수량)는 order-service 자신의 inventories가 단일 진실
 * 공급원이라 이 포트로 가져오지 않는다.
 */
public interface RewardCatalogClient {

    List<RewardSnapshot> getRewards(UUID projectId);
