 ...
 * 지워지면 재입고/쿠폰만료 알림이 조용히 유실되고 아웃박스를 둔 이유가 사라진다.
 */
@SpringBootTest
@Testcontainers(disabledWithoutDocker = true)
@TestPropertySource(properties = {
        "internal-api.key=test-only-internal-api-key",
        "notification-outbox.poll-interval-ms=3600000",
        // 닿지 않는 주소. KafkaConfig의 max.block.ms(5초)가 상한이라 오래 매달리지 않는다.
        "spring.kafka.bootstrap-servers=localhost:1"})
@Transactional
class NotificationOutboxIntegrationExceptionTest {
    ...
    private List<NotificationOutboxJpaEntity> unpublished() {
    ...
    void 브로커에_닿지_못하면_미발행으로_남고_시도횟수가_오른다() {
        // given
        notificationPublisher.publishRewardRestocked(
                new OrderNotificationPublisher.RewardRestockedEvent(1L, UUID.randomUUID()));
