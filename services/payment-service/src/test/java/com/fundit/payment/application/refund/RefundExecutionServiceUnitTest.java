...
class RefundExecutionServiceUnitTest {
    ...
    void 판매자_승인_부분취소는_전액이_아니므로_보류금을_해제하지_않는다() {
        // given
        Payment payment = completedPayment();
        RefundRequest refundRequest = RefundRequest.requestAfterShipment(RefundTriggerType.DEFECT, FUNDING_ID, payment.getId(), UUID.randomUUID(), "파손", java.util.List.of("url"));
