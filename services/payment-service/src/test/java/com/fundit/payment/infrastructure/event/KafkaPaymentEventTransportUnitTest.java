...
class KafkaPaymentEventTransportUnitTest {
    ...
    void 결제완료_발행시_payment_completed_토픽으로_fundingId를_키로_보낸다() {
        // given
        setUp();
        var event = new PaymentEventTransport.PaymentCompletedTransportEvent(1024L, 7L);
        ...
        verify(kafkaTemplate).send(eq(KafkaTopics.PAYMENT_COMPLETED), eq("1024"), payloadCaptor.capture());
        Map<String, Object> payload = payloadCaptor.getValue();
        assertThat(payload.get("eventId")).isEqualTo("payment:42");
        assertThat(payload.get("fundingId")).isEqualTo(1024L);
    ...
    void 환불완료_발행시_refund_completed_토픽으로_모든_필드를_채워_보낸다() {
        // given
        setUp();
        var event = new PaymentEventTransport.RefundCompletedTransportEvent(2048L, 9L, "CANCELLED_BY_MEMBER", true);
        ...
        // then
        ArgumentCaptor<Map<String, Object>> payloadCaptor = ArgumentCaptor.forClass(Map.class);
        verify(kafkaTemplate).send(eq(KafkaTopics.REFUND_COMPLETED), eq("2048"), payloadCaptor.capture());
        Map<String, Object> payload = payloadCaptor.getValue();
        assertThat(payload.get("eventId")).isEqualTo("payment:77");
        assertThat(payload.get("fundingId")).isEqualTo(2048L);
