 ...
 * 도메인 서비스(RefundExecutionService 등)를 거치지 않고 아웃박스 행을 직접 적재해
 * 이 테스트를 PG 연동/암복호화 흐름과 분리한다.
 *
 * <p>여기서 확인하는 것: 전송이 실패하면 행이 <b>발행된 것처럼 지워지지 않는다</b>.
 * 지워지면 결제완료/환불완료 이벤트가 조용히 유실되고 아웃박스를 둔 이유가 사라진다.
 */
@SpringBootTest
...
@Transactional
class PaymentEventOutboxIntegrationExceptionTest {
    ...
    private List<PaymentEventOutboxJpaEntity> unpublished() {
    ...
    void 브로커에_닿지_못하면_미발행으로_남고_시도횟수가_오른다() {
        ...
        outboxRepository.save(PaymentEventOutboxJpaEntity.builder()
                ...
                .payload(Map.of("couponIssuanceIds", List.of(7L)))
                .build());
