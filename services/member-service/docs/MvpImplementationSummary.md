...
2. ~~**임시 인증 방식**: 게이트웨이가 아직 없어 `X-Account-Id` 헤더를 서명 검증 없이 그대로 신뢰~~ **→ 2026-09-07 해소됨(`feat/gateway-service#21`).** 게이트웨이(`platform:gateway-service`)가 JWT를 검증해 `X-User-Id`/`X-User-Roles`를 주입하고, 클라이언트가 보낸 같은 이름의 헤더는 게이트웨이에서 제거한다. 서비스 쪽은 `modules:common-webmvc`의 `@LoginUser CurrentUser`로 주입받고(`CurrentMemberArgumentResolver`/`CurrentMember`/`InternalApiKeyFilter`/`WebConfig` 삭제), 게이트웨이를 우회한 직접 호출은 `InternalGatewaySecretFilter`가 `X-Internal-Api-Key`로 차단한다. 설계·보안 근거는 `platform/gateway-service/docs/` 참고.
