...
## 이 서비스가 하는 일
회원 프로필, 구매자/판매자 접근(별도 모드를 지정하지 않고 가입 시 양쪽 모두 자동 부여), 약관동의(이력 보존), 찜, 판매자 팔로우, 배송지 소유. (`PRD.md` 2. 도메인/서비스 개요 기준)
...
리워드 품절 알림 신청, 닉네임 수정은 MVP 범위 밖입니다. (팔로우는 2026-09-16 #58에서 구현됐습니다 — PM이 MEMBER-006을 "찜한 프로젝트 목록과 찜한 판매자 목록"으로 바꿔 팔로우 목록이 필요해졌습니다.) **소셜가입 전용 엔드포인트(`POST /members/social`)는 만들지 않기로 확정**했습니다 — 기존 `POST /members`의 요청 본문에 비밀번호도 소셜 관련 필드도 없어 provider와 무관하기 때문입니다(소셜 가입도 이 엔드포인트를 그대로 씁니다) — 상세는 `MvpImplementationSummary.md` 참고.
...
## 에러 코드
도메인 전용 코드는 `MemberErrorCode implements ErrorCode`로 만든다(서비스당 flat enum 1개 — `error-handling.md` 컨벤션). `INVALID_INPUT`/`UNAUTHORIZED`/`NOT_FOUND`/`DEPENDENCY_FAILURE`는 이미 `CommonErrorCode`에 있으니 재정의하지 않는다. member-service 고유의 도메인 에러 코드는 현재 없음 — 팔로우의 본인 팔로우(400)·없는 회원(404)도 `CommonErrorCode.INVALID_INPUT`/`NOT_FOUND`로 충분하다.
