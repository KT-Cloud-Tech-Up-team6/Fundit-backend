...
class RewardUnitExceptionTest {
    ...
    void 한정수량인데_수량이_없으면_예외가_발생한다() {
        // when & then
        assertThatThrownBy(() -> Reward.create(1L, "얼리버드", "설명", null, 39000L, true, null, false, null, null, null))
    ...
    void 무제한인데_수량이_있으면_예외가_발생한다() {
        // when & then
        assertThatThrownBy(() -> Reward.create(1L, "얼리버드", "설명", null, 39000L, false, 10, false, null, null, null))
                .isInstanceOf(BusinessException.class)
                .extracting(e -> ((BusinessException) e).getErrorCode())
                .isEqualTo(ProjectErrorCode.INVALID_REWARD_QUANTITY);
    }
    ...
    void 수정시에도_수량_정합성을_재검증한다() {
        ...
        // when & then
        assertThatThrownBy(() -> reward.changeBasicInfo("이름", "설명", null, 39000L, true, null, false, null, null, null))
                .isInstanceOf(BusinessException.class)
                .extracting(e -> ((BusinessException) e).getErrorCode())
                .isEqualTo(ProjectErrorCode.INVALID_REWARD_QUANTITY);
    }
    ...
    void 얼리버드인데_할인정보가_없으면_예외가_발생한다() {
        // when & then
        assertThatThrownBy(() -> Reward.create(1L, "얼리버드", "설명", null, 39000L, false, null, true, null, null, null))
                .isInstanceOf(BusinessException.class)
                .extracting(e -> ((BusinessException) e).getErrorCode())
    ...
    void 정액할인이_가격_이상이면_예외가_발생한다() {
        ...
        assertThatThrownBy(() -> Reward.create(1L, "얼리버드", "설명", null, 39000L, false, null, true,
                EarlyBirdDiscountType.AMOUNT, 39000L, null))
                .isInstanceOf(BusinessException.class)
                .extracting(e -> ((BusinessException) e).getErrorCode())
                .isEqualTo(ProjectErrorCode.INVALID_EARLY_BIRD_DISCOUNT);
    }
    ...
    void 정률할인이_100을_초과하면_예외가_발생한다() {
        // when & then
        assertThatThrownBy(() -> Reward.create(1L, "얼리버드", "설명", null, 39000L, false, null, true,
                EarlyBirdDiscountType.RATE, 101L, null))
                .isInstanceOf(BusinessException.class)
                .extracting(e -> ((BusinessException) e).getErrorCode())
                .isEqualTo(ProjectErrorCode.INVALID_EARLY_BIRD_DISCOUNT);
    }
