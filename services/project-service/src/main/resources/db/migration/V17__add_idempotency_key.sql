...
-- deleted_at IS NULL도 같이 걸어야 한다 — 안 걸면 삭제된(soft-deleted) DRAFT가 키를 영구
-- 점유해서, 애플리케이션의 재요청 조회(findBySellerIdAndIdempotencyKeyAndDeletedAtIsNull)는
-- 못 찾는데 DB 유니크 제약은 여전히 막아 가짜 CONFLICT가 난다.
