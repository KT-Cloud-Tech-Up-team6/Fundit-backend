...
## 이 서비스가 하는 일
프로젝트, 리워드/옵션, 커뮤니티, 새소식, 후기. (`PRD.md` 2. 도메인/서비스 개요 기준)

프로젝트 심사(관리자 승인)는 **폐지됐다** — 필수 작성 항목이 채워지면 관리자 승인 없이 바로 공개(ONGOING)로 전환한다.
...
- projects — id(BIGINT), public_id(UUID v7, 외부 노출용 — URL/API 경로 파라미터는 전부 이 값), seller_id(UUID, member-service members.id 참조, FK 아님 — 서비스 분리), business_type(GENERAL/SOLE/CORP), category_major/category_minor(FK), title(40자 제한), goal_amount(50만원 이상 CHECK), funding_start_at/funding_deadline(둘 다 nullable — 공개(발행) 시점에만 확정, 그 전엔 NULL), status(DRAFT/ONGOING/SUCCEEDED/FAILED), deleted_at(소프트 딜리트), project_display_code(GENERATED ALWAYS AS 'F' || LPAD(id,7,'0') — 애플리케이션에서 직접 세팅하지 않음).
- project_open_notify_requests — 오픈 예정 프로젝트 알림 신청. (project_id, member_id) 유니크(중복 신청 방지).
- rewards — project_id(FK), price, is_limited/quantity(is_limited=true면 quantity 필수·0 이상, false면 quantity는 반드시 NULL — DB CHECK로 강제), has_option, simple_refund_disabled, is_early_bird/early_bird_discount_type(AMOUNT/RATE)/early_bird_discount_value(is_early_bird=false면 둘 다 반드시 NULL — DB CHECK로 강제), deleted_at(소프트 딜리트, 삭제된 리워드는 소비자 응답에서 제외), reward_display_code(GENERATED, R0000001 형식). category_type/disclosure 컬럼은 V9 마이그레이션으로 제거됨(법정고시 MVP 범위 제외).
...
- funding_start_at/funding_deadline은 생성 시점에 확정하지 않는다: 공개(발행, PROJECT-029) 처리 로직에서만 값을 채운다. 그 전 단계 API에서 이 값을 요구하거나 임의로 세팅하지 않는다.
...
- **프로젝트 공개/수정은 search-service 색인에 이벤트로 통지해야 한다(SEARCH-011)**: 필수항목 완료로 인한 공개 전환(`ProjectService.submit`)은 `project.approved.v1`을, 공개된(`Project.isPublic()`) 프로젝트의 `updateBasicInfo`/`updateStory`는 `project.updated.v1`을 `ProjectIndexEventPublisher`로 발행한다 — `RewardEventPublisher`와 동일한 아웃박스+Kafka 패턴(`ProjectIndexEventOutboxWorker`/`KafkaProjectIndexEventTransport`). DRAFT 단계의 수정은 발행하지 않는다(애초에 검색 색인에 없는 프로젝트). `sellerDisplayName`은 `SellerProfileClient`로 채우는데, 이 클라이언트가 아직 `NoopSellerProfileClient`라 지금은 항상 `null`로 나간다.
...
- 개인정보 수집 동의 없이 공개(발행) 등 다음 단계로 진행시키지 말 것 (PRIVACY_CONSENT_REQUIRED)
- 미공개(DRAFT) 프로젝트를 공개 상세 API에서 404가 아닌 다른 코드로 구분하지 말 것 (존재 여부 비노출)
...
- `ProjectService.DEFAULT_FUNDING_PERIOD`(`application/project/ProjectService.java`) — 공개(발행, PROJECT-029) 시 `funding_deadline`을 계산하는 모금기간 기본값을 30일로 가정해뒀다. PRD/API 명세서 어디에도 이 값이 명시돼 있지 않아 코드 상수로만 고정한 상태 — 기획 확정 필요. 값이 확정되면 이 상수만 바꾸면 된다.