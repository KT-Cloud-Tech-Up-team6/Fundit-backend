...
class EmailMaskerUnitTest {
    ...
    void 로컬파트가_짧으면_통째로_가린다(String email, String expected) {
        // given — 가릴 자리가 없으면 한 글자도 흘리지 않는다
        // when
        String masked = EmailMasker.mask(email);
        // then
        assertThat(masked).isEqualTo(expected);
    ...
    void 도메인에_골뱅이가_여러_개면_마지막을_기준으로_나눈다() {
        // given — 로컬파트에 따옴표로 @를 넣을 수 있다(RFC 5321)
        // when
        String masked = EmailMasker.mask("ab@cdefg@fundit.com");
        // then
        assertThat(masked).isEqualTo("ab@cd***@fundit.com");
    }

    @Test
    void 골뱅이가_없으면_전부_가린다() {
        // given — 저장 시점에 검증하므로 방어용이다
        // when
        String masked = EmailMasker.mask("not-an-email");
        // then
