 ...
 *
 * <p>{@code auth.encryption.key}는 Base64로 인코딩된 AES-256 키(32바이트)여야 하며,
...
public class AesGcmCipher {
    ...
    public AesGcmCipher(@Value("${auth.encryption.key}") String base64Key) {
        byte[] keyBytes = Base64.getDecoder().decode(base64Key);
        if (keyBytes.length != 32) {
            throw new IllegalStateException(
                    "auth.encryption.key는 AES-256용 32바이트 키여야 합니다(현재 %d바이트).".formatted(keyBytes.length));
        }
